...
def check_existing_generated_files():
...
if "prompt" not in st.session_state:
    st.session_state.prompt = load_text_from_file("saved_prompt.txt")
...
if menu == "Prompt & Generate":
    ...
    if ip:
        ...
        save_text_to_file("saved_ip.txt", st.session_state.custom_ip)
    ...
    if not st.session_state.gpt_response:
        ...
        existing_files = check_existing_generated_files()
        if existing_files:
...
if menu == "Container Management":
    ...
    if network_mode == "Use existing network":
        try:
            ...
            save_network_name(selected_network_name)
        except Exception as e:
    ...
    if network_mode == "Create new network":
        ...
        Docker will assign IPs to containers within this range.
        """)
        with st.form("create_new_net_form"):
            ...
            if subnet:
                try:
                    gateway_ip = calculate_gateway_ip(subnet)
    ...
    with col1:
        if st.button("Stop Container"):
            container_id = check_container_exists(container_name)
            if container_id:
...
if menu == "Help":
    ...
    st.markdown("""
    **Home:** View machine and container status.  
    **Prompt & Generate:** Describe your scenario and generate a honeypot.  
    **Container Management:** Manage Docker container, image, and networks.  
    **Tip:** Docker must be running.
    """)
